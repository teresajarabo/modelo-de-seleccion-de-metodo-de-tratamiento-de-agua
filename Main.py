...
with seleccion_modelo:
    ...
    with col3:
        st.markdown('Contaminantes en el agua:')
        st.markdown(f'* **Presencia microbiológica:** {calidad[0]}')
        st.markdown(f'* **Presencia química:** {calidad[1]}')
    ...
    new_df = new_df.loc[(new_df['Caudal'] >= caudal/n_maquinas) & 
                        (new_df['Capacidad microbiologica'] >= calidad[0]) & 
                        (new_df['Capacidad quimica'] >= calidad[1]) & 
